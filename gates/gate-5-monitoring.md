...
| 3 | Instrumentación real probada | bloque `frescura` en `bcv-ingest estado` (79 tests en verde; verificado contra el corpus real: última jornada 2026-07-10, 2 días); [corrida verde de `health.yml`](https://github.com/jeremialcala/bcv-fx-ingestor/actions/runs/29219899597) con el guard de `WORKER_URL` actuando según diseño | ✅ |
...
Gate aprobado el 2026-07-12: `[Unreleased]` cortado a **`1.0.0`** (primer release productivo,
cierre del ciclo AI-DLC), `pyproject.version` sincronizado, tag `v1.0.0` publicando la imagen
GHCR (`v1.0.0` + `latest`) y CronJob de K8s actualizado al tag nuevo.
