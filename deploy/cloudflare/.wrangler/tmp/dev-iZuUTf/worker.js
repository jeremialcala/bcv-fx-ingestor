...
// worker.js
var OBJETO = "bcv_fx.db";
var worker_default = {
  async fetch(request, env) {
    if (request.method !== "GET" && request.method !== "HEAD") {
      return new Response("m\xE9todo no soportado", { status: 405, headers: { allow: "GET, HEAD" } });
    }
    const url = new URL(request.url);
    if (url.pathname === "/bcv_fx.db") {
      const objeto = await env.BUCKET.get(OBJETO);
      if (objeto === null) {
        return new Response("artefacto no publicado todav\xEDa", { status: 404 });
      }
...
// .wrangler/tmp/bundle-N4F19g/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-N4F19g/middleware-loader.entry.ts
